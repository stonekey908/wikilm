...
export async function POST(request: NextRequest) {
  ...
  const { prompt, projectCwd, projectId, type, title } = body;

  if (!prompt || !projectCwd || !projectId || !type || !title) {
    return Response.json(
      { error: "Missing required fields: prompt, projectCwd, projectId, type, title" },
      { status: 400 }
    );
  }
