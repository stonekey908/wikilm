...
- **Web clipper (browser bookmarklet).** One-click clip from any webpage into a chosen project. A tiny bookmarklet loads `clip.js` from localhost; the script posts the page HTML to `/api/clip`, the server runs Turndown to produce real markdown, and the source lands as `pending` for triage. **No web grounding required** — captures the page as you see it (auth-walled, dynamic, SPA content all work). On the success page, pick a destination project and click Save; on `/sources`, pending rows get **Preview** + project picker for triage before approving. Run the dev server with `npm run dev:https` (one-time cert trust) so the bookmarklet works from HTTPS pages. Sites with strict `script-src` CSP will block the injection — for those, use a real browser extension like MarkDownload. Full setup in [SETUP.md](SETUP.md#web-clipper-browser--wikilm).
